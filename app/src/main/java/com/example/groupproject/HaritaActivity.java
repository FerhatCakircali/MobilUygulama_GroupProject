...
public class HaritaActivity extends AppCompatActivity {
    ...
    public boolean isServicesOK(){//Google play hizmetleri kontrolü
        Log.d(TAG, "isServicesOK: google hizmetleri sürümünü kontrol etme");
        ...
        if (available== ConnectionResult.SUCCESS){
            ...
            Log.d(TAG,"isServicesOK: Google Play Hizmetleri çalışıyor");
            return true;
        }
        else if(GoogleApiAvailability.getInstance().isUserResolvableError(available)){
            //bir hata oluştu ama çözebiliriz
            Log.d(TAG,"isServicesOK: bir hata oluştu ama düzeltebiliriz");
            Dialog dialog= GoogleApiAvailability.getInstance().getErrorDialog(HaritaActivity.this,available,ERROR_DIALOG_REQUEST);
            dialog.show();
        }
        else {
            Toast.makeText(this,"Harita istekleri yapamazsınız",Toast.LENGTH_SHORT).show();
        }
        return false;
    }
